...
const TimelineSection = ({ isDesktop }: IDesktop) => {
  ...
  const addText = (
    ...
    const subtitleString = subtitle
      ? `<p class='text-md mt-2 text-gray-200 font-medium tracking-wide'>${subtitle}</p>`
      : "";
  ...
  const renderSectionTitle = (): React.ReactNode => (
    <div className="flex flex-col">
      ...
      <h1 className="section-heading seq mt-2">Experiences</h1>
      <h2 className="text-2xl md:max-w-2xl w-full seq mt-2">
        A quick walkthrough of my professional journey
      </h2>
    </div>
  );
